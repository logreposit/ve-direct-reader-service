...
class LogrepositIngressDataMapperTests {
    ...
    @Test
    fun `test map from VeDirectReadings to IngressData, given ignored fields, expect ignored fields are filtered out`() {
        val now = Instant.now()

        val ingressData = LogrepositIngressDataMapper(LogrepositConfiguration().also { it.ignoredFields = listOf("serial_number") }).toLogrepositIngressDto(
            date = now,
            address = "1",
            data = sampleVeDirectReadings()
        )
        ...
        assertThat(fields[5].name).isEqualTo("alarm_state")
        assertThat(fields[5].datatype).isEqualTo(DataType.INTEGER)
        assertThat((fields[5] as IntegerField).value).isEqualTo(1)

        assertThat(fields[6].name).isEqualTo("relay_state")
        assertThat(fields[6].datatype).isEqualTo(DataType.INTEGER)
        assertThat((fields[6] as IntegerField).value).isEqualTo(0)
    }
